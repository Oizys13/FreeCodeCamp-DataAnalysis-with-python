...
class TestCalculations(unittest.TestCase):
    def test_first_calculation(self):
        result = mean_var_std.perform_calculations([2, 6, 2, 8, 4, 0, 1, 5, 7])
        expected_result = {
        ...
        self.assertAlmostEqual(result, expected_result, "Mismatch between calculated and expected values for input '[2,6,2,8,4,0,1,5,7]'")

    def test_second_calculation(self):
        result = mean_var_std.perform_calculations([9, 1, 5, 3, 3, 3, 2, 9, 0])
        expected_result = {
        ...
        self.assertAlmostEqual(result, expected_result, "Mismatch between calculated and expected values for input '[9,1,5,3,3,3,2,9,0]'")
    
    def test_incorrect_list_size(self):
        with self.assertRaises(ValueError) as e:
            mean_var_std.perform_calculations([2, 6, 2, 8, 4, 0, 1])
        self.assertEqual(str(e.exception), "Input list must contain exactly nine elements.", "Expected ValueError for incorrect list size")
